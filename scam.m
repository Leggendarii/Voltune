...
inner_loop = collect(-I_V1/V1, "s");
fprintf('\nThe inner loop transfer function G1 is:  \n');
pretty(inner_loop)

%% Outer loop (V) transfer function
outer_loop_V = collect(v_3/(-I_V1), "s");
fprintf('\nThe outer transfer function G2 (in abc) is:  \n');
pretty(outer_loop_V)

fprintf('\nThe outer transfer function G2 (in qd) is:  \n');

syms R1 R2 L1 L2 C3 w s real
num = expand(R2 + L2*(s+1i*w));
den = expand(C3*L2*(s+1i*w)^2 + C3*R2*(s+1i*w) + 1);

dx = real(expand(num*(1i)));
sx = real(expand(den));

collect(eval(sx), "s");

outer_loop_V_dq = dx/sx;

pretty(outer_loop_V_dq)

%% Outer loop (P) transfer function
i_2 = v_4/R2;

outer_loop_P = i_2/(-I_V1);

fprintf('\nThe outer transfer function G3 (in abc) is:  \n');

syms V_30 real

pretty(outer_loop_P*V_30)
...
outer_loop_P_dq = dx/sx;

pretty(outer_loop_P_dq)

%% PLL plant

fprintf('\nThe PLL plant transfer function (in abc) is:  \n');

pretty(collect(v_3/V1, "s"))

fprintf('\nThe PLL plant transfer function (in dq) is:  \n');
s = (s+1i*w);

num = expand((R2 + L2*s));
den = expand(1i*(R1 + R2 + L1*s + L2*s + C3*L1*L2*s^3 + C3*L1*R2*s^2 + C3*L2*R1*s^2 + C3*R1*R2*s));

num = imag(num);
den = collect(imag(den), "s");
